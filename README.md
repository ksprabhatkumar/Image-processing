...
│
├── input_raw_images/      # Place your .ARW, .CR2, .NEF files here
├── output_jpg_images/     # Processed images from main.py will appear here
└── output_baseline_images/ # Processed images from baseline.py will appear here
```
...
To run the high-performance, multi-threaded version with the intelligent enhancement pipeline:
```bash
python main.py
```
...
To run the simple, sequential version with no filters:
```bash
python baseline.py
```
The processed JPGs will be saved in the `output_baseline_images/` folder.
